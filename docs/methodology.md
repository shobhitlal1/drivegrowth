...
The generator was calibrated once during development after the first run revealed implausibly low acquisition-cost assumptions. Published screenshots, reports and the database use the final configuration above. These figures are illustrative inputs, not external benchmarks.
...
This approximates 24 months of contribution for a newly acquired policy. It does not include acquisition costs. The full snapshot supplies these descriptive assumptions even when viewing earlier acquisition cohorts; this is not a point-in-time backtest. Constant survival smooths the annual renewal discontinuity. It does not establish predictive accuracy, causal lift, or diminishing returns. A held-out predictive model is a potential extension outside the current scope.
